...
- **5.0.438**: A new **`Clone Masters Container`** duplicate-path resolver (`Clonemasterscontainer`) for the pooled-masters topology — many distinct TD clone masters gathered under one shared parent COMP (e.g. `/TOX_CLONE_MASTERS/Surface_Master`, `/TOX_CLONE_MASTERS/basic_scene`, …) with their instances scattered elsewhere. When a duplicate-path group has **exactly one** member at or beneath that container, it's auto-selected as master and the rest tagged `clone` with no prompt. This is the durable fix for a topology the existing **`Template Master Name`** convention can't express: that rule matches a unique whole-segment **name** (one template, copies elsewhere), whereas this matches **ancestor-container membership** (descendant-of-container), so it stays correct even though every master shares the same `TOX_CLONE_MASTERS` segment. Wired into `checkForDuplicates` (`_resolveByMastersContainer`) after the native TD clone/replicant resolvers and ahead of the name convention; empty disables it, 0 or 2+ matches fall through to the manual prompt; persisted via `config.json`. Plus **Verbose duplicate-group diagnostics**: with Verbose on, `checkForDuplicates` logs each group's members (TD path, `par.clone` target, `isClone`/`isInsideClone`/`isReplicant`) and names the resolver that claimed it, and `_buildPathGroups` logs each operator it excludes as a clone/replicant — the fast path to "which resolver is my group actually taking, and why?". New `TestMastersContainerResolution` covers single/nested/zero/multiple/empty matches and end-to-end no-prompt resolution.
